...
private:
  ...
  std::ifstream m_syncMapFile;
  std::vector<unsigned int> m_lowSync;
  std::vector<unsigned int> m_upSync;
...
public:
  ...
  bool removeEvents(unsigned int nevent);
  void readSyncMap(const char* filename);
