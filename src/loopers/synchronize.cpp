...
bool Synchronize::removeEvents(unsigned int nevent)
{
  for(unsigned int i = 0; i < m_lowSync.size(); i++){
    if( (nevent>=m_lowSync.at(i)) && (nevent<=m_upSync.at(i)) ){
      /// message
      if( (nevent==m_lowSync.at(i)) || (nevent==m_upSync.at(i)) ) {
        cout << endl << "reading event: " << nevent << endl;
        if(nevent==m_upSync.at(i)) {
          m_lowSync.erase(m_lowSync.begin());
          m_upSync.erase(m_upSync.begin());
        }
      }
      /// end message
      return false;
...
void Synchronize::readSyncMap(const char* filename)
{
  m_syncMapFile.open(filename);
  unsigned int a,b;
  while (m_syncMapFile >> a >> b){
    m_lowSync.push_back(a);
    m_upSync.push_back(b);
  }
}
  ...
  for (unsigned int nevent = _startEvent; nevent <= _endEvent; nevent += _eventSkip)
  {
    ...
    if(SYNCMAP){
      if (removeEvents(nevent+1)) {
        ...
        cout << "written event: " << nevent+1 << endl;
        totalWrittenEvents++;
        continue;
      }
    }
    ...
    {
      cout << endl << "suppresing deletion of invalid event: " << nevent+1 << endl;
      if(!SYNCMAP){
        delete refEvent;
        delete dutEvent;
        invalidEvents++;
    ...
    if (sync.getFull())
    {
      ...
      {
        questionableEvents++;
        std::cout << std::endl << "target diff larger then threshold: " << sync.getTargetDiff() << " at event: " << nevent << std::endl;
      }
      else
      {
  ...
  m_upSync()
{
  ...
  if(SYNCMAP){
    cout << "WARNING!!!!!!!!!!! synchronization from map is turned on!!!!!!!!!!" << endl;
    readSyncMap("syncMaps/ref-map.dat");
    cout << "sync map read!" << endl;
    for(unsigned int i = 0; i < m_lowSync.size(); i++){
      cout << "lowBound: " << m_lowSync.at(i) << " upBound: " << m_upSync.at(i) << endl;
    }
  }
}
