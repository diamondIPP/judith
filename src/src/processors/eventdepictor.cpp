...
void EventDepictor::depictTrack(const Storage::Track* track)
{
  ...
  for (unsigned int ncluster = 0; ncluster < track->getNumMatchedClusters(); ncluster++) {
    if (track->getMatchedCluster(ncluster)->getMatchedTrack() != track ){
      std::cout << "DepictTrack: matched cluster track didn't associated" << std:: endl;
      std::cout << "::EventDepictor::" << std::endl << 
           " track         : " << track                                                 << std::endl << 
           " matchedCluster: " << track->getMatchedCluster(ncluster)                    << std::endl <<
           " matchedTrack  : " << track->getMatchedCluster(ncluster)->getMatchedTrack() << std::endl <<
           "::EventDepictor::" << std::endl;
      return;
    }
  }

  std::cout << "depicting track " << track << std::endl;
  ...
  if (!can) can = new TCanvas(name, title, 800, 700);
  can->Divide(1, 2);
