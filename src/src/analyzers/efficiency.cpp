                              ...
                              const Storage::Event* dutEvent)
{
  ...
  for (unsigned int ntrack = 0; ntrack < refEvent->getNumTracks(); ntrack++)
  {
    ...
    //std::cout << "relativeToSensor: " << _relativeToSensor << std::endl;
    if (_relativeToSensor >= 0 && !matches.at(_relativeToSensor)) {
      cout << "ASDASDASD" << endl;
      continue;
    }
    /*if (_relativeToSensor > 0 && !matches.at(_relativeToSensor)) {
      std::cout << "// If asking for relative efficiency to a plane" << std::endl;
      continue;
    }*/
