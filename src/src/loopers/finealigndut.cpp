...
void FineAlignDut::loop()
{
  for (unsigned int niter = 0; niter < _numIterations; niter++)
  {
    ...
    std::cout << "---numPixX--- " << numPixX << std::endl;
    std::cout << "---binxPerPix--- " << binxPerPix << std::endl;
    ...
    for (unsigned int nsens = 0; nsens < _dutDevice->getNumSensors(); nsens++)
    {
      ...
      std::cout << " ==== residualAlignment ended ====" << std::endl;
      std::cout << "sensor: " << nsens << " getOffY: " << sensor->getOffY() << std::endl;
      std::cout << "sensor: " << nsens << " offsetY: " << offsetY << std::endl;
      std::cout << "sensor: " << nsens << " getOffX: " << sensor->getOffX() << std::endl;
      std::cout << "sensor: " << nsens << " offsetX: " << offsetX << std::endl;
      std::cout << "sensor: " << nsens << " z rotation: " << rotation << std::endl;
    }
  }
