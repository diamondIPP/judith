...
int KartelConvert::readHits()
{
  ...
  while (!frameEnded())
  {
    ...
    if (!_line.sync && VERBOSE2){
      cout << "sync: " <<_line.sync << endl;
      cout << "triggerOffset: " <<_line.triggerOffset << endl;
      cout << "triggerNumber: " <<_line.triggerNumber << endl;
      for (unsigned int i = 0; i < _numPlanes; i++){
        cout << "ch0[" << i << "]: " << std::hex << _line.ch0[i] << std::dec << endl;
        cout << "ch0[" << i << "]: " << std::hex << _line.ch1[i] << std::dec << endl;
      }
    }
...
int KartelConvert::readFrameInfo()
{
  ...
  /*cout << "  triggerFlag = " << _frame.triggerFlag << endl; 
  cout << "  triggerOffset = " << _frame.triggerOffset << endl; // check the mapping
  cout << "  _line.sync = " << _line.sync << endl; // check the mapping
  getchar();*/
...
int KartelConvert::SetMapping(const ConfigParser& config)
{
  ...
  for (unsigned int i = 0; i < config.getNumRows(); i++)
  {
    ...
    if (row->isHeader && !row->header.compare("End Readout"))
    {
      if(nrows!=16){cout << "kartelconvert:   WARNING: there seems to be a problem with reading the mapping info." << endl;}
      return 0;
    }
    ...
    if (row->header.compare("Readout"))
      continue; // Read only the chosen mapping

    if (!row->key.compare("sync"))
      {_bitMap.sync = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("trigger"))
      {_bitMap.trigger = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("triggerOffset"))
      {_bitMap.triggerOffset = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("timeStamp"))
      {_bitMap.timeStamp = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch00"))
      {_bitMap.ch0[0] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch10"))
      {_bitMap.ch1[0] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch01"))
      {_bitMap.ch0[1] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch11"))
      {_bitMap.ch1[1] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch02"))
      {_bitMap.ch0[2] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch12"))
      {_bitMap.ch1[2] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch03"))
      {_bitMap.ch0[3] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch13"))
      {_bitMap.ch1[3] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch04"))
      {_bitMap.ch0[4] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch14"))
      {_bitMap.ch1[4] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch05"))
      {_bitMap.ch0[5] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else if (!row->key.compare("ch15"))
      {_bitMap.ch1[5] = ConfigParser::valueToNumerical(row->value); nrows++;}
    else
      throw "Converter: can't parse mapping row";
  }
  ...
  _start_event(startEvent)
{
  ...
  _bitMap.sync          = _bitMap.sync          + 8 - 16 *          (_bitMap.sync > 7);
  _bitMap.trigger       = _bitMap.trigger       + 8 - 16 *       (_bitMap.trigger > 7);
  _bitMap.triggerOffset = _bitMap.triggerOffset + 8 - 16 * (_bitMap.triggerOffset > 7);
  _bitMap.timeStamp     = _bitMap.timeStamp     + 8 - 16 *     (_bitMap.timeStamp > 7);
  _bitMap.ch0[0]        = _bitMap.ch0[0]        + 8 - 16 *        (_bitMap.ch0[0] > 7);
  _bitMap.ch1[0]        = _bitMap.ch1[0]        + 8 - 16 *        (_bitMap.ch1[0] > 7);
  _bitMap.ch0[1]        = _bitMap.ch0[1]        + 8 - 16 *        (_bitMap.ch0[1] > 7);
  _bitMap.ch1[1]        = _bitMap.ch1[1]        + 8 - 16 *        (_bitMap.ch1[1] > 7);
  _bitMap.ch0[2]        = _bitMap.ch0[2]        + 8 - 16 *        (_bitMap.ch0[2] > 7);
  _bitMap.ch1[2]        = _bitMap.ch1[2]        + 8 - 16 *        (_bitMap.ch1[2] > 7);
  _bitMap.ch0[3]        = _bitMap.ch0[3]        + 8 - 16 *        (_bitMap.ch0[3] > 7);
  _bitMap.ch1[3]        = _bitMap.ch1[3]        + 8 - 16 *        (_bitMap.ch1[3] > 7);
  _bitMap.ch0[4]        = _bitMap.ch0[4]        + 8 - 16 *        (_bitMap.ch0[4] > 7);
  _bitMap.ch1[4]        = _bitMap.ch1[4]        + 8 - 16 *        (_bitMap.ch1[4] > 7);
  _bitMap.ch0[5]        = _bitMap.ch0[5]        + 8 - 16 *        (_bitMap.ch0[5] > 7);
  _bitMap.ch1[5]        = _bitMap.ch1[5]        + 8 - 16 *        (_bitMap.ch1[5] > 7);
