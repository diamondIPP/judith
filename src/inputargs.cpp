...
InputArgs::InputArgs() :
  ...
  _cfgDUT(""),
  _cfgTestbeam(""),
  _cfgReadout("configs/readout/CERN.cfg"),
  _command(""),
  _numEvents(0),
  _eventOffset(0),
  _cfgReadoutSet(false),
  _noBar(false)
{ }
...
void InputArgs::usage()
{
  ...
  cout << "  -m  " << setw(w2) << "--cfgReadout" << " : path to readout configuration\n";
...
void InputArgs::parseArgs(int* argc, char** argv)
{
  ...
  if (*argc > 1)
  {
    for ( int i = 1; i < *argc; i++ )
    {
      ...
      else if ( (!arg.compare("-m") || !arg.compare("--cfgReadout")) &&
                !_cfgReadout.compare("configs/readout/CERN.cfg") )
      {
        _cfgReadout = argv[++i];
        _cfgReadoutSet = true;
        cout << setw(w) << "  cfgReadout" << " : " << _cfgReadout << endl;
      }
    ...
    // Default readout mapping
    if ( (!_command.compare("convert") && _cfgReadoutSet == false ))
    {
      cout << setw(w) << "  cfgReadout" << " : " << _cfgReadout << endl;
    }
  }
...
string InputArgs::getCfgReadout() const { return _cfgReadout; }
